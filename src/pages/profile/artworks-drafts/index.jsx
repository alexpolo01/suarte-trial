...
export default function ProfileArtworkDrafts() {
  const { profileData, setProfileData } = useContext(ProfileDataContext);
  const { loading, fetchData, setFetchData, loadMoreData } = useCache(`${profileData._id}_artwork_drafts`, `${config.apis.api.url}/drafts`, {
    injectToken: true, 
    invalidateWhen: ["ARTWORK_DRAFT_SAVED", "NEW_ARTWORK_REQUEST", `${profileData._id}_REFRESH`],
  ...
  return (
    <>
      <div className="artworks-drafts__go-back" onClick={()=>navigate("/profile/artworks", { replace: true, preventScrollReset: true })}>
        <BackArrowIcon className="artworks-drafts__go-back-icon"/>
                
        <Text className="artworks-drafts__go-back-text" medium paragraph>
                    Artworks
        </Text>
      </div>
      ...
      {
        loading ? 
          <CustomSpinner className="artwork-drafts__loading" thin/> 
          : fetchData.length === 0 ? 
            <Text className="artwork-drafts__no-drafts" paragraph small>
                        No artwork drafts available.
            </Text>
            :
            <div className="artwork-drafts__container">
              <InfiniteScroll 
                          ...
                          <List
                            ...
                            rowRenderer={({ index, key, style }) => (
                              <div key={key} style={style} ref={infiniteScrollAssigner(index)}>
                                <div 
                                  className="artwork-drafts__draft element-non-selectable" 
                                  onClick={()=>(
                                    navigate("/profile/add-artwork", {
                                      state: {
                                ...
                                >
                                  <RemoveIconCircle 
                                    className="artwork-drafts__remove-draft" 
                                    onClick={(e)=>{
                                      e.stopPropagation(); 
                                  ...
                                  <Text className="artwork-drafts__draft-title dots-on-overflow" medium>
                                    {fetchData.data[index].draft_container.artwork_about.artwork_title}
                                  </Text>

                                  <ForwardArrowIcon className="artwork-drafts__enter-draft"/>
                                </div>
                              </div>
                            )}
                          />
