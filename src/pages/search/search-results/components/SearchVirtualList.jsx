...
import './styles/SearchVirtualList.css';

export default function SearchVirtualList({ items, onLoadMore, navigateToArtwork }) {
  ...
  function initColumnSizesCache() {
    const columnCount = screenSize.width <= 692 ? 2 : screenSize.width <= 1400 ? 3 : 4;
    const spacer = screenSize.width <= 600 ? 10 : 15;
    const widthOfList = screenSize.width <= 900 ? screenSize.width - 22 : 
      screenSize.width <= 1004 ? screenSize.width - 371: 
        screenSize.width <= 1300 ? screenSize.width - 395 : 
          Math.min(screenSize.width - 455, 1355);
  ...
  return (
    <>
      <InfiniteScroll 
        ...
        className="search-virtual-list-infinite"
      >
