...
export default function VirtualList({ items, onLoadMore }) {
  ...
  return (
    <>
      <InfiniteScroll 
        className="search-users-virtual-list remove-scrollbar" 
        onLoadMore={onLoadMore} 
        shouldLoadMore={items.data.length < items.totalDocs} 
        offsetElements={3} 
        listLength={items.data.length} 
        defaultInfiniteLoaderClassName="search-users-virtual-list-loader"
      >
              ...
              <AutoSizer disableHeight>
                {({ width }) => (
                  <List
                    ...
                    rowHeight={screenSize.width <= 550 ? 58 : 63}
                    scrollTop={scrollTop}
                    overscanRowCount={3}
                    ref={listRef}
                    rowRenderer={({ index, key, style }) => (
                      <div key={key} style={style} ref={infiniteScrollAssigner(index)}>
                        <Row data={items.data[index]}/>
                      </div>
                    )}
                  />
                )}
