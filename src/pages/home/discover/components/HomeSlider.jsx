...
export default function HomeSlider({ artworks, alternativeArtworks, children, category, endpoint, total, link, hideText=false }) {
  const navigateToArtwork = useNavigateToArtwork(`${category}_collection_page`, { totalDocs: total, data: alternativeArtworks ? alternativeArtworks : artworks }, endpoint);
    ...
    <>
      <div className="home-slider__container element-non-selectable">
        <div className="home-slider__header">
          ...
          <Text className="home-slider__see-all" medium onClick={()=>navigate(link, { state: { from: true } })}>
                        See all
          </Text>
        </div>
