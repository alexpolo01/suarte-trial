...
export default function VirtualList({ items, onLoadMore, addTag, formState }) {
              ...
              <AutoSizer disableHeight>
                {({ width }) => (
                  <List
                    ...
                    rowHeight={screenSize.width <= 550 ? 54 : 61}
                    scrollTop={scrollTop}
                    overscanRowCount={5}
                    ref={listRef}
                    rowRenderer={({ index, key, style }) => (
                      <div key={key} style={style} ref={infiniteScrollAssigner(index)}>
                        <Row data={items.data[index]} addTag={addTag} formState={formState}/>
                      </div>
                    )}
                  />
                )}
