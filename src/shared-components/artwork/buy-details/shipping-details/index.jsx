...
export default function ShippingDetails({ artworkData, formState, setFormState, typeOfPurchase }) {
  const { loading, fetchData, setFetchData } = useCache(`artwork_${artworkData._id}_payment_details_${typeOfPurchase}`, `${config.apis.api.url}/payment/${artworkData._id}`, {
    injectToken: true,
    expiresIn: ["2", "minutes"],
  ...
  async function continueToCheckout() {
    ...
    const { response, data } = await UserService.createCheckoutSession(artworkData._id, formState, typeOfPurchase);
            ...
            : openReviews ?
              ...
              <>
                <div className="purchase-shipping-details__shipped-by-container">
                  ...
                  <span className="purchase-shipping-details__add-adress element-non-selectable mt-s" onClick={()=>setOpenAddAddressForm(true)}>
                ...
                {fetchData.valorations.total !== 0 && (
                  <div className="purchase-shipping-details__reviews-details">
                    ...
                    <span className="purchase-shipping-details__add-adress element-non-selectable mt-s" onClick={()=>setOpenReviews(true)}>
...

                {
                  fetchData.user_addresses.length === 0 ?
                    <p className="purchase-shipping-details__empty-text mt-s">
                                        Please add a shipping address by clicking on the "Add shipping address" button above.
                    </p>
                    :
                    fetchData.user_addresses.map(address => (
                      <ShippingCard
                        ...
                        selectShipping={()=>setFormState({ ...formState, shipping_details: address })}
