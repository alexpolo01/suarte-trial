...
export default function BuyLimitedEditionButton({ artworkData }) {
  ...
  const privateActionHandler = useProtectAction({
    typeOfProtection: "USER_ACCOUNT_REQUIRED",
  });
  const areLimitedEditionsSold = useMemo(() => {
    if (artworkData.artwork_limited_editions) {
      for (const key in artworkData.artwork_limited_editions) {
        ...
        if (
          Boolean(limitedEditionSize) &&
          limitedEditionSize.sold < limitedEditionSize.quantity
        ) {
          return false;
        }
      }
  ...
  if (!artworkData.artwork_limited_editions) {
    return (
      <>
        <div
          className="buy-limited-edition-button__container element-non-selectable not-available"
    ...
    return <LimitedEditionsSold artworkData={artworkData} />;
  } else {
    return (
      <>
        <div
