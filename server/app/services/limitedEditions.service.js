...
function calculateLimitedEditionsPriceAndUnits(editions, originalPrice) 
{
    for (let i = 0; i < editions.length; i++) 
    {
        ...
        editions[i].quantity = UNITS[editions.length - 1][i + 1];
        editions[i].sold = 0;
    }
}
...
function calculateLimitedEditions(height, width, price) 
{
    ...
    if(editionSmall)  
    {
        editions.push(editionSmall);
    }
    if(editionMedium) 
    {
        editions.push(editionMedium);
    }
    if(editionLarge)  
    {
        editions.push(editionLarge);
    }
...
module.exports = calculateLimitedEditions;