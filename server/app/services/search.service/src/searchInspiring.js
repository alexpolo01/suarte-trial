async function searchInspiring(req, model) 
{
    const searchQuery = req.query.username ?? "";
    const followee_id = req.params.user_id ?? "";
    ...
    results.data = results.data.map(async (user) => 
    {
        ...
        {
            details.im_following = await model.isFollowing(req.token_decoded.uid, user.user_details._id);
            details.is_following_me = await model.isFollowing(user.user_details._id, req.token_decoded.uid);
        }
        return details;
    });
